...
import CustomDonut, {
  DonutDataType,
} from '@/components/molecules/customDonut/customDonut';
import {
  ExtractedDataList,
...
type Props = {
  ...
  data: GetAssetAllocationModel;
  isPrimary: boolean;
};

function OrionAsset(props: Props) {
  ...
  const [orionChartList, setOrionChartList] = useState<DonutDataType[]>([]);
  ...
  /** Added by @Tarun 24-03-2025 -> Render legend item using flash list (FYN-5971) */
  ...
  return (
    <View style={styles.cardMain}>
      <View style={styles.orionAssetLay}>
        <CustomText
          maxFontSizeMultiplier={maxFontSize}
          color={theme.colors.assetCardText}
          variant={TextVariants.bodyLarge}
        ...
        {props.data?.status == 1 ? (
          <View style={styles.orionAssetMain}>
            <CustomDonut
              data={orionChartList}
      ...
      >
        <View>
          ...
          <View pointerEvents="none" style={styles.donutLay}></View>
...
const makeStyles = (theme: CustomTheme) =>
  StyleSheet.create({
    ...
    orionAssetMain: { flex: 1, justifyContent: 'center', alignItems: 'center' },
    donutLay: {
