...
try {
  ...
  console.error(
    '❌ unzipper package not found. Please install it with: npm install unzipper',
  );
  process.exit(1);
}
...
// Download and extract tenant ZIP from API
function downloadAndExtractTenant(appName, extractPath) {
  return new Promise((resolve, reject) => {
    const url = `http://13.232.196.97:3000/api/automation/tenant?appName=${encodeURIComponent(
      appName,
    )}`;
...
if (!appName) {
  console.error(errorSymbol + 'Please provide an appName.');
  process.exit(1);
}

const projectRoot = process.cwd();
const currentTenantFolder = path.join(projectRoot, 'currentTenant');
const tenantInfoPath = path.join(
  currentTenantFolder,
  appName,
  'tenantInfo.json',
);
...
(async () => {
  try {
    console.log(
      successSymbol +
        `Downloading and extracting tenant for appName: ${appName}...`,
    );

    // Download and extract ZIP to currentTenant folder
    await downloadAndExtractTenant(appName, currentTenantFolder);
    console.log(successSymbol + `Tenant extracted to ${currentTenantFolder}.`);

    // Read tenantInfo.json from extracted files
    if (!fs.existsSync(tenantInfoPath)) {
      throw new Error(
        `tenantInfo.json not found in extracted ZIP at ${tenantInfoPath}. Ensure the ZIP download completed successfully.`,
      );
    }

    const tenantInfo = JSON.parse(fs.readFileSync(tenantInfoPath, 'utf8'));
    console.log(successSymbol + `Tenant info loaded.`);

    // Continue with the rest of the script
    const tenantFolderPath = path.join(currentTenantFolder, appName);
    runUpdateTenant(tenantInfo, tenantFolderPath);
  } catch (error) {
    console.error(errorSymbol + `Failed to download tenant: ${error.message}`);
    process.exit(1);
  }
})();

function runUpdateTenant(tenantInfo, tenantFolder) {
  const tenantName = tenantInfo.AppName.replace(/\s+/g, '');
  ...
  // ------------------------------------
  const {
    newVersionName: IosNewVersionName,
    newVersionCode: IosNewVersionCode,
  ...
  const fontDirPath = path.join(tenantFolder, 'assets', 'fonts', 'AppFont');
